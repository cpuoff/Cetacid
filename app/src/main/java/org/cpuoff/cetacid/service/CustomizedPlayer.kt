...
import org.cpuoff.cetacid.data.getUnshuffledIndex
import org.cpuoff.cetacid.data.setUnshuffledIndex
...
class CustomizedPlayer(val inner: ExoPlayer) : ForwardingPlayer(inner) {
    ...
    fun disableShuffle() {
        ...
        if (unshuffledIndex == null) {
            Log.e("Cetacid", "Current track has no unshuffled index when disabling shuffle")
            replaceMediaItems(
                0,
                itemCount,
